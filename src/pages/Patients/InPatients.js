...
export default function InPatients() {
  ...
  const save = () => {
    if (!form.name) return;
    setPatients(prev => [...prev, {...form, status:'Active'}]);
    setShowModal(false);
    setForm({patientId:'',name:'',ward:'General',doctor:'',admittedDate:'',bed:''});
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">🛏️ In-Patient Management</div><div className="page-sub">Currently admitted patients</div></div>
        <button className="btn btn-primary" onClick={()=>setShowModal(true)}>+ Admit Patient</button>
      </div>
      <div className="search-bar">
        <div className="search-input">🔍 <input placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)}/></div>
      </div>
      <div className="card"><div className="card-body" style={{padding:0}}><div className="table-wrap">
        ...
        <tbody>{filtered.length ? filtered.map((p,i)=>{
          const days = p.admittedDate ? Math.floor((new Date()-new Date(p.admittedDate))/86400000) : 0;
          return (
            <tr key={i}>
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label>Ward</label>
                <select value={form.ward} onChange={e=>setForm({...form,ward:e.target.value})}>
                  <option>General</option><option>ICU</option><option>Surgical</option><option>Maternity</option>
                </select>
              </div>
